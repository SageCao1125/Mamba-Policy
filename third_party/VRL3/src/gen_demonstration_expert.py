...
def parse_args():
    ...
    parser.add_argument('--save_reward', default=False, action='store_true', help='save reward')
...
def main():
    ...
    if args.save_reward:
        save_dir = os.path.join(args.root_dir, 'adroit_'+args.env_name+'_expert_reward.zarr')
    else:
    ...
    while episode_idx < num_episodes:
        ...
        while (not time_step.last()) or step_count < minimal_episode_length:
            ...
            # cprint(f"Step {step_count}, Reward: {time_step.reward}, Goal Achieved: {time_step.n_goal_achieved}", 'green')
    ...
    reward_arrays = np.array(reward_arrays)[:, np.newaxis]
    # print('reward_ends_arrays:', reward_ends_arrays)
    ...
    if args.save_reward:
        zarr_data.create_dataset('reward', data=reward_arrays, dtype='float32', overwrite=True, compressor=compressor)
