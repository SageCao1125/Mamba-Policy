...
class AdroitRunner(BaseRunner):
    def __init__(self,
        ...
        self.logger_util_test1 = logger_util.LargestKRecorder(K=1)
    ...
    def run(self, policy: BasePolicy):
        ...
        for episode_idx in tqdm.tqdm(range(self.eval_episodes), desc=f"Eval in Adroit {self.task_name} Pointcloud Env",
            ...
            while not done:
                ...
                with torch.no_grad():
                    ...
                    obs_dict_input['img'] = obs_dict['img'].transpose(1,3).unsqueeze(0)  ## [B,C,H,W]-->[B,1,H,W,C]       ## new add rgb 24/06/19
                    obs_dict_input['depth'] = obs_dict['depth'].unsqueeze(0)
                ...
                action = np_action_dict['action'].squeeze(0)      ## [n_action_steps-1, action_dim]
        ...
        self.logger_util_test1.record(np.mean(all_success_rates))
        self.logger_util_test.record(np.mean(all_success_rates))
        self.logger_util_test10.record(np.mean(all_success_rates))
        log_data['SR_test_L1'] = self.logger_util_test1.average_of_largest_K()
